...
import { Form, Row, Col, Card, Button, Input, Tabs, Modal, message } from 'antd';
...
export default class DetailFormInfo extends PureComponent {
  ...
  renderFormItem = () => {
    const { selectMenuTypeValue } = this.state;
    const { formItems, dispatch, form, showModalType, currentItem } = this.props;
    return formItems.map((item, i) => {
      ...
      return (
        <Col lg={item.colSpan === 0 ? 0 : item.colSpan || 8} md={item.colSpan === 0 ? 0 : 12} sm={item.colSpan === 0 ? 0 : 24} key={`${item.key}_${i}`} >
          <FormItem
            label={`${item.label}`}
            {...formItemLayout}
            hasFeedback
  ...
  render() {
    const { modalVisible, parentKey, currentKey, selectMenuTypeValue } = this.state;
    return (
