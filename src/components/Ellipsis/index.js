...
const EllipsisText = ({ text, length, tooltip, ...other }) => {
  ...
  if (tooltip) {
    return <Tooltip title={text} overlayStyle={{ wordBreak: 'break-all' }}><span>{displayText}{tail}</span></Tooltip>;
  }
    ...
    return (
      ...
      >
        ...
        >
          {
            tooltip ? (
              <Tooltip title={text} overlayStyle={{ wordBreak: 'break-all' }}>{childNode}</Tooltip>
            ) : childNode
          }
