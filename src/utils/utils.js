  ...
  (function dataFormater(menuData) {
    menuData.forEach(item => {
      if (item.children) {
        ...
        // 判断menu path页面是否跳转到新页面，添加新页面访问权限
        const detailPath = detailPagePathList.find(info => {
          return info.key === `/${item.path}`
        });
        detailPath && list.push(...detailPath.detailPathChild);
        list.push(`/${item.path}`)
      }
    })
  })(menuData)
...
export function formItemAddInitValue(formItems, currentItem) {
  ...
  if (currItemKeys.length > 0) {
    formItems.forEach(item => {
      ...
      if (index > -1) {
        newFormItems.push(Object.assign(item, { initialValue: currentItem[currItemKeys[index]] }));
      } else {
    ...
    formItems.forEach(item => {
      newFormItems.push(Object.assign(item, { initialValue: undefined }));
    })
  }
